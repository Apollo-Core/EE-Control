...
class GraphTransformWhileTest {
  ...
  void testGraphModification() {
    ...
    int numFunctionNodes = (int) input.getVertices().stream().filter(node -> TaskPropertyService.isProcess(node)).count();
    ...
    numFunctionNodes = (int) input.getVertices().stream().filter(node -> TaskPropertyService.isProcess(node)).count();
